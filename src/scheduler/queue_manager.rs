...
use crate::models::{
    ChargingMode, ChargingPile, ChargingRequest, PileStatus, PILE_QUEUE_CAPACITY,
...
impl QueueManager {
    ...
    pub async fn add_pile(&self, pile: Arc<ChargingPile>) {
        ...
        pile_queues.insert(
            pile.number.clone(),
            VecDeque::with_capacity(PILE_QUEUE_CAPACITY),
        );
    ...
    // 从等候区移除请求
    pub async fn remove_from_waiting_queue(
        &self,
        request_id: Uuid,
    ...
    pub async fn get_waiting_count(&self, mode: ChargingMode) -> usize {
        ...
        queue.iter().filter(|r| r.mode == mode.to_string()).count()
    }
...
mod tests {
    ...
    async fn test_pile_queue_operations() {
        ...
        // 测试添加到充电桩队列
        assert!(manager
            .add_to_pile_queue(&pile.number, request.clone())
            .await
            .is_ok());
        ...
        // 测试移除请求
        let removed = manager
            .remove_from_pile_queue(&pile.number, request.id)
            .await;
        assert!(removed.is_some());
        assert_eq!(removed.unwrap().id, request.id);
    }
