...
use sqlx::MySqlPool;
use uuid::Uuid;
use charging_station::get_next_queue_number;
use chrono::Utc;
use std::sync::Mutex;
...
pub async fn create_charging_request(
    ...
    // 获取当前等待数量
    let waiting_count = requests.iter()
        .filter(|r| r.mode == payload.mode && r.status == RequestStatus::Waiting)
        .count();
        ...
        match request.status {
            ...
            RequestStatus::Charging => {
                return HttpResponse::BadRequest()
                    .json(ApiResponse::<()>::error("充电中不允许修改模式，请先取消充电"));
            }
            _ => {
                return HttpResponse::BadRequest()
...
pub async fn update_charging_amount(
    ...
    if let Some(request) = requests.iter_mut().find(|r| r.id == request_id) {
        match request.status {
            ...
            RequestStatus::Charging => {
                return HttpResponse::BadRequest()
                    .json(ApiResponse::<()>::error("充电中不允许修改充电量，请先取消充电"));
            }
            _ => {
                return HttpResponse::BadRequest()
...
pub async fn get_user_charging_requests(
    ...
    // 过滤出当前用户的充电请求
    let requests = CHARGING_REQUESTS.lock().unwrap()
        .iter()
...
pub async fn get_charging_queue(
    ...
    let mut requests = CHARGING_REQUESTS.lock().unwrap()
        .iter()
        .filter(|req| req.mode == mode && req.status == RequestStatus::Waiting)
        .cloned()
        .collect::<Vec<_>>();
